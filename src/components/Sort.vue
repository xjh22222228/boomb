<template>
  <el-dropdown placement="bottom-end" :hide-on-click="false">
    ...
    <template #dropdown>
      <el-dropdown-menu>
        <el-dropdown-item @click="isUp = true">
          <el-icon :class="{ transparent: !isUp }"><Check /></el-icon>
          {{ t('asc') }}
        </el-dropdown-item>
        <el-dropdown-item @click="isUp = false">
          <el-icon :class="{ transparent: isUp }"><Check /></el-icon>
          {{ t('desc') }}
        </el-dropdown-item>

        <el-dropdown-item divided @click="sortType = SortType.FileSize">
          <el-icon :class="{ transparent: sortType !== SortType.FileSize }"
            ><Check
          /></el-icon>
          {{ t('fileSize') }}
        </el-dropdown-item>
        <el-dropdown-item @click="sortType = SortType.FileName">
          <el-icon :class="{ transparent: sortType !== SortType.FileName }"
            ><Check
          /></el-icon>
          {{ t('fileName') }}
        </el-dropdown-item>
...
const isUp = ref(true)
const dir = computed<IFile[]>(() => store.getCachedDir(route))
...
watch([sortType, isUp], () => {
  ...
  switch (sortType.value) {
    case SortType.FileSize:
      sortDir = dir.value.sort(
        (a: IFile, b: IFile) => (a.size ?? 0) - (b.size ?? 0),
      )
      break
